...
为 [Arcane Angler](https://arcaneangler.com/) 提供自动抛竿、自动打 Boss、验证码检测和消息通知功能。

### 功能

- 自动查找并点击“抛竿线”按钮。
- 使用可配置的随机操作间隔，支持分别设置小间隔、大间隔和大间隔概率。
- 在抛竿请求发出前将 payload 中的 `isTrusted` 修改为 `true`。
- 检测到页面出现“人机验证”滑块或 “Staff Question” 时默认自动尝试完成验证；Staff Question 仅自动回答能够可靠解析的基础算术题。
- 自动过验证失败、遇到无法可靠回答的问题或该功能关闭时，会停止自动操作，并支持通过 Server酱或浏览器通知提醒。
- 在收益面板中统计鱼获、金币等收益，收获分类使用中文名称和稀有度颜色展示。
- 可复用游戏自身的角色、比赛、每日任务和天气响应自动选择已解锁地图；可选追逐金风和优先每日任务，优先级为公会锦标赛、个人比赛、金风、每日任务、天气与地图等级评分。
- 可分别配置常规、个人赛和公会赛使用的鱼饵等级，自动检查当前地图对应鱼饵的库存，低于阈值时按 100 或 1000 个自动购买并装备；金币不足时临时切换免费默认饵，金币足够后自动恢复购买。
- 可自动检测活动中的世界 Boss，结合角色装备后属性和 Boss 弱点选择预估伤害最高的招式，并按 6 秒冷却持续攻击。
...
- 自动保存启停状态、自动过验证、自动换图、鱼饵补货、自动打 Boss、通知方式、定时休息和面板折叠设置。
...
- “自动换地图”默认关闭；开启后仅在自动抛竿运行时从已解锁地图中选择，组队时仅队长会自动切换整队地图。
- “自动买鱼饵”默认关闭；开启后仅在自动抛竿运行时检查库存，并会配合自动换地图购买目标地图对应等级的鱼饵。
- “自动打 Boss”默认关闭；开启并启动脚本后会在任意游戏页面检测并攻击活动中的世界 Boss。
...
- 在“设置”页中可调整每次点击前的小间隔、大间隔时间范围及大间隔概率；默认为 0.5～2 秒、5～10 秒和 8%。
- 在“设置”页中可将地图等级加权量设为 0%、5% 或 10%；评分公式为“天气经验加成 +（地图编号 - 1）× 加权量”。
- “优先比赛地图”设置默认开启；只匹配已经报名的个人比赛和当前公会已参加的锦标赛，公会锦标赛优先级更高。
- “追逐金风”设置默认关闭；开启后在无可用比赛地图时优先选择金风天气的已解锁地图。
- “优先每日任务”设置默认关闭；开启后在无可用比赛和金风地图时，按未完成每日任务的目标地图与天气条件筛选已解锁地图，再选择经验评分最高项。
- 在“设置”页中可分别选择常规、个人赛、公会赛和金风天气使用的默认、低级、中级、高级或超级饵；金风鱼饵默认为免费默认饵。
- 在“设置”页中可设置 1～100000 的鱼饵库存阈值和每次购买 100 / 1000 个；还可调整连续未钓鱼自动刷新的等待分钟数。
